...
async function getLinks(id) {
  ...
  data.pvs.forEach((platform) => {
    ...
    if (!allowed.includes(platform.service.toLowerCase())) {
      return console.log(platform.service);
    }
...
async function callVoca(url) {
  let language = chrome.storage.sync.get('language');
  try {
    let lang = "&lang="+language
    if (language == "Default") {lang = "&lang=English"}
    const query = `https://vocadb.net/api/songs?query=${url}&childTags=false&unifyTypesAndTags=false&childVoicebanks=false&includeMembers=false&onlyWithPvs=false&start=0&maxResults=10&getTotalCount=false&preferAccurateMatches=false&fields=ThumbUrl${lang}`;
    const response = await fetch(query);
    if (!response.ok) {
      throw new Error(`Response status: ${response.status}`);
    }
